...
def load_data_traj(run, cations, polymers, num_cation, run_start, run_end, cutoff_radii):
    ...
    poly_n = np.zeros((run_end - run_start, num_cation))  # 初始化锂所在聚合物链索引的数组
    li_positions = np.zeros((run_end - run_start, num_cation, 3))  # 初始化氧坐标的数组
...
def compute_msd(dt, li_positions, oe_ave_n, poly_n, run_start, run_end, threshold):
    msd_in_dt = []
    count_li_dt = []
    if dt == 0:
...
def compute_dn_msd_parallel(li_positions, oe_ave_n, poly_n, run_start, run_end, time_window, dt, dt_collection, threshold=0.05):
    ...
    with ProcessPoolExecutor() as executor:
        # 使用字典存储future以保持顺序
        futures = {executor.submit(compute_msd, dt, li_positions, oe_ave_n, poly_n, run_start, run_end, threshold): dt
...
if __name__ == '__main__':
    ...
    li_positions, oe_ave_n, poly_n, = load_data_traj(run_unwrap, cations_unwrap, polymers_unwrap, num_cation,
                                                     run_start, run_end, r_li_peo)
    ...
    # 写入CSV文件
    df.to_csv('msd_M1.csv', index=False)
